...
### Added
- **PDF export rebuilt as a branded, print-ready document**, replacing the previous minimal single-page PDF. `saveToPdf(filename, options?)` now accepts `PdfExportOptions` (`title`, `subtitle`, `xLabel`, `yLabel`, `footerLeft`, `footerRight`, `theme: 'light' | 'dark'`) independent of the on-screen chart's own title/axis labels — `xLabel`/`yLabel` are applied to the live chart's `Settings` store just long enough to rasterize the export image, then restored, so the on-screen chart is never visibly affected.
  - A4 landscape page, sized and proportioned (page/chart/header/footer ratios) from a Claude Design proposal (`Chart Export Page.dc.html`), built as real off-screen HTML/CSS and captured with `html2canvas` — no Node-dependent PDF renderer involved, so it works in any browser/WebView the library itself runs in. The screenshot is embedded directly into a single-page PDF via the original dependency-free `buildPdf`, which now reads the JPEG's own real pixel dimensions (rather than assuming a fixed px-per-pt ratio) so the embedded image's declared size always matches its actual resolution.
  - Captured at 3x resolution (`html2canvas`'s `scale` option) so the export stays crisp at print size regardless of the on-screen chart's pixel density; the chart image is fit into its page region with plain `max-width`/`max-height` + intrinsic aspect ratio — no skew, no manual sizing math.
  - Typeset in IBM Plex Sans / IBM Plex Mono via Google Fonts (matching the design), loaded natively by the browser. The INFICON wordmark/icon logo is inlined as base64 data URIs (`src/lib/export/assets.ts`) rather than referenced from `public/`, since the library build (`publicDir: false`) and NuGet package don't ship anything from there — a Blazor consumer's wwwroot would otherwise have no file for the export to resolve.
